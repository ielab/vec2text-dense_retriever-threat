...
class Corrector(BaseTrainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.inversion_trainer.model.noise_level = 0.0  # we do the noise for hypothesis in corrector.
    ...
    def evaluation_loop(
        ...
        if self.args.str_mix_augmentation or not self.model.config.use_frozen_embeddings_as_input:
            self.inversion_trainer.model.to(self.args.device)
        else:
    ...
    def _precompute_hypothesis_and_embedding(
        ...
        if self.model.config.use_frozen_embeddings_as_input:
            ...
            if filter_correct_examples:
                ...
                def embedding_is_not_correct(ex):
                    if "frozen_embeddings" in ex:
                        ...
                        hypothesis_text = self.tokenizer.batch_decode(ex['hypothesis_input_ids'],
                                                                      skip_special_tokens=True)
                        target_text = self.tokenizer.batch_decode(ex['input_ids'],
                                                                  skip_special_tokens=True)
                        return [t1 != t2 for t1, t2 in zip(hypothesis_text, target_text)]
    ...
    def _get_hypothesis_uncached(self, inputs: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if 'hypothesis_input_ids' in inputs:
    ...
    def compute_loss(
        ...
        if self.args.str_mix_augmentation and self.model.training:
            print(f"Using str_mix_augmentation {self.args.str_mix_augmentation}")
            new_inputs_ids, new_attention_mask, new_labels = self.inversion_trainer.mix_string_batch(inputs['embedder_input_ids'], inputs['labels'].shape[1])

            (
                new_frozen_embeddings,
                new_hypothesis_input_ids,
                new_hypothesis_attention_mask,
                new_hypothesis_embedding,
            ) = self._get_hypothesis_uncached({'input_ids': self.tokenizer(self.embedder_tokenizer.batch_decode(new_inputs_ids),
                                                                           return_tensors='pt',
                                                                           add_special_tokens=False,
                                                                           padding=True)['input_ids'].to(new_inputs_ids.device),
                                               'embedder_input_ids': new_inputs_ids,
                                               'attention_mask': new_attention_mask})
            inputs['frozen_embeddings'] = torch.cat([inputs['frozen_embeddings'], new_frozen_embeddings], dim=0)
            inputs['hypothesis_input_ids'] = torch.cat([inputs['hypothesis_input_ids'], new_hypothesis_input_ids], dim=0)
            inputs['hypothesis_attention_mask'] = torch.cat([inputs['hypothesis_attention_mask'], new_hypothesis_attention_mask], dim=0)
            inputs['hypothesis_embedding'] = torch.cat([inputs['hypothesis_embedding'], new_hypothesis_embedding], dim=0)
