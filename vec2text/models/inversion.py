...
from tevatron.modeling import DenseModel, SpladeModel
...
class InversionModel(transformers.PreTrainedModel):
    ...
    training_embedding_noise_level: float
    ...
    def __init__(self, config: InversionConfig):
        ...
        try:
            training_embedding_noise_level = config.training_embedding_noise_level
        ...
        if embedder_model_api:
            ...
            bottleneck_dim = self.embedder_dim
        elif isinstance(embedder, DenseModel):
            if embedder.pq_index is not None:
                self.embedder_dim = embedder.pq_index.code_size
            elif embedder.pooler is None:
                self.embedder_dim = embedder.lm_p.pooler.dense.out_features
            else:
                self.embedder_dim = embedder.pooler.output_dim
            bottleneck_dim = self.embedder_dim
        elif isinstance(embedder, SpladeModel):
            self.embedder_dim = embedder.lm_p.cls.predictions.decoder.out_features
            bottleneck_dim = self.embedder_dim
        else:
    ...
    def call_embedding_model(
        ...
        elif isinstance(self.embedder, DenseModel) or isinstance(self.embedder, SpladeModel):
            embeddings = embedder.encode_passage(
                {"input_ids": input_ids, "attention_mask": attention_mask}
            )
        else:
        ...
        if self.eval_embedding_noise_level is not None:
            embeddings += self.eval_embedding_noise_level * torch.randn(
    ...
    def embed_and_project(
        ...
        if (self.training) and (self.training_embedding_noise_level > 0):
            embeddings += self.training_embedding_noise_level * torch.randn(
                embeddings.shape, device=embeddings.device
            )
