...
def load_nq_dpr_corpus() -> datasets.Dataset:
    ...
    # data_path = os.path.join(out_dir, 'nq-train')
    # if not os.path.exists(data_path):
    #     data_path = util.download_and_unzip(url, out_dir)
    # data = GenericDataLoader(data_path)
    # corpus, queries, qrels = data.load(split='train')
    ...
    # for q in qrels:
    #     q_ctx = queries[q]
    #     data_dict["text"].append(q_ctx)

    # query_dataset = Dataset.from_dict(data_dict)
    # pass_dataset['train'] = concatenate_datasets([pass_dataset['train'], query_dataset])
    return pass_dataset
...
def load_nq_val() -> datasets.Dataset:
    d = load_nq_dpr_corpus()["dev"]
    return d
...
def load_nq_val_datasets() -> datasets.DatasetDict:
    """Loads a pre-defined set of standard val datasets."""
    d = {
        'nq': load_nq_val(),
    }
