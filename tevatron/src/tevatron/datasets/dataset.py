...
class HFTrainDataset:
    def __init__(self, tokenizer: PreTrainedTokenizer, data_args: DataArguments, cache_dir: str):
        ...
        if 'msmarco' in data_args.dataset_name:
            self.no_title = True
        else:
            self.no_title = False
    ...
    def process(self, shard_num=1, shard_idx=0):
        ...
        if self.preprocessor is not None:
            self.dataset = self.dataset.map(
                self.preprocessor(self.tokenizer, self.q_max_len, self.p_max_len, self.separator, self.no_title),
                batched=False,
...
class HFCorpusDataset:
    ...
    def process(self, shard_num=1, shard_idx=0):
        ...
        if self.preprocessor is not None:
            self.dataset = self.dataset.map(
                self.preprocessor(self.tokenizer, self.p_max_len, self.separator, self.no_title),
                batched=False,
                num_proc=self.proc_num,
                remove_columns=self.dataset.column_names,
                desc="Running tokenization",
