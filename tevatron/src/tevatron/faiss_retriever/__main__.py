...
from .retriever import BaseFaissIPRetriever, FaissRetriever, FaissRetrieverLoad
...
def main():
    ...
    if 'index.faiss' in index_files[0]:
        logger.info(f'Exist faiss index is found {index_files[0]}')
        faiss_index = faiss.read_index(index_files[0])
        retriever = FaissRetrieverLoad(faiss_index)
        lookup_path = index_files[0].split('/')
        lookup_path[-1] = 'lookup.pkl'
