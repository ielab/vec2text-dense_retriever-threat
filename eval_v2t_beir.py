...
from vec2text.data_helpers import dataset_from_args, load_nq_val_datasets, load_ag_news_test, load_nq_val, load_beir_datasets
...
val_datasets_dict = load_beir_datasets()
...
for dataset in ['nfcorpus', 'scidocs', 'scifact', 'trec-covid']:
    eval_results = trainer.evaluate(
        eval_dataset=val_datasets[dataset]
    )

    for key in ['eval_bleu_score', 'eval_token_set_f1', 'eval_exact_match', 'eval_emb_cos_sim']:
        print(dataset, key, eval_results[key])
