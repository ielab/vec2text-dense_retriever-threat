...
--query_reps embedings/query/nq/dim_768/query_emb.pkl \
--passage_reps embedings/corpus/nq/dim_768/'corpus_emb.*.pkl' \
--query_save_to embedings/query/nq/dim_768_quantization_m768/ \
--passage_save_to embedings/corpus/nq/dim_768_quantization_m768/ \
--quantization \
--m 768

query_dir=embedings/query/nq/dim_768_quantization_m768/
corpus_dir=embedings/corpus/nq/dim_768_quantization_m768/
result_dir=dr_results/nq
dim=768_quantization_m768
...
--batch_size 128 \
--save_text \
--save_ranking_to ${result_dir}/run.nq.dim_${dim}.txt

python -m tevatron.utils.format.convert_result_to_trec \
              --input ${result_dir}/run.nq.dim_${dim}.txt \
              --output ${result_dir}/run.nq.dim_${dim}.trec

python -m pyserini.eval.convert_trec_run_to_dpr_retrieval_run \
              --topics dpr-nq-test \
              --index wikipedia-dpr \
              --input ${result_dir}/run.nq.dim_${dim}.trec \
              --output ${result_dir}/run.nq.dim_${dim}.json

python -m pyserini.eval.evaluate_dpr_retrieval \
                --retrieval ${result_dir}/run.nq.dim_${dim}.json \
                --topk 10 20 100 1000 > ${result_dir}/run.nq.dim_${dim}.metrics
'''
